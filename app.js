...
// Hàm reset mật khẩu cho admin (xóa mật khẩu đảng viên)
async function adminResetPassword(cccd) {
  ...
  if (msgEl) {
    msgEl.innerHTML = `<div class="msg" style="background:#f0f8f0;color:#2a7a2a;">⏳ Đang xóa mật khẩu...</div>`;
  }
  ...
  try {
    ...
    if (result.success) {
      if (msgEl) {
        msgEl.innerHTML = `<div class="msg success">✅ ${result.message || "Xóa mật khẩu thành công!"}</div>`;
      }
      
      // Đóng modal và tải lại dữ liệu sau 0.5s
      setTimeout(async () => {
        closeEditModal();
        await loadAllMembers();
        renderTable();
      ...
      if (msgEl) {
        msgEl.innerHTML = `<div class="msg error">❌ ${result.message || "Có lỗi xảy ra khi xóa mật khẩu."}</div>`;
      }
    }
  } catch (err) {
    if (msgEl) {
      msgEl.innerHTML = `<div class="msg error">❌ Lỗi kết nối: ${err.message}</div>`;
    }
  }
}
    ...
    if (f === "Mật khẩu" && isAdmin && !isAddMode) {
      ...
      inputHtml = `
        ...
        <div style="margin-top:8px;display:flex;align-items:center;flex-wrap:wrap;gap:8px;">
          <button class="btn btn-danger" style="font-size:12px;padding:6px 16px;min-height:34px;" 
