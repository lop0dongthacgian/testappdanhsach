...
async function saveSheetConfig() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Làm mới dữ liệu hiển thị. Nếu bước làm mới này lỗi vì lý do nào đó,
      // việc lưu cấu hình vẫn đã thành công nên cửa sổ cài đặt vẫn phải đóng lại
      // (người dùng có thể bấm nút "🔄 Tải lại dữ liệu" ở màn hình chính để thử lại).
      try {
        await loadAllMembers();
        renderTable();
      } catch (refreshErr) {
      ...
      // Tự động đóng modal sau 1.2 giây
      setTimeout(() => {
...
async function adminResetPassword(cccd) {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (currentEdit) {
        // Reload lại dữ liệu cho đảng viên hiện tại
