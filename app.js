...
async function doMemberLogin() {
  ...
  showLoading("Vui lòng chờ, đang tải dữ liệu!");
  try {
...
function openChangePasswordModal() {
  ...
  // Ô mật khẩu (MatKhau) trống trong sheet ⇒ đảng viên chưa đặt mật khẩu ⇒
  // làm mờ và bỏ yêu cầu nhập "Mật khẩu hiện tại".
  let hasCurrentPassword = true;
  if (session.role === "member") {
    const currentMember = members[0];
    hasCurrentPassword = !!(currentMember && String(currentMember.MatKhau || "").trim() !== "");
  }
