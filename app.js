...
async function saveSheetConfig() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Cập nhật displayHeaders ngay sau khi lưu thành công
      // để form chỉnh sửa sử dụng tên cột mới
      ...
      loadAllMembers()
        ...
        .catch(refreshErr => {
          ...
          alert("⚠️ Lưu cấu hình thành công nhưng có lỗi khi tải lại dữ liệu. Vui lòng bấm nút 'Tải lại dữ liệu' để cập nhật.");
        })
        .finally(() => hideLoading());
    } else {
...
function buildEditForm(record) {
  ...
  // Sử dụng displayHeaders đã được cập nhật (tên cột mới)
