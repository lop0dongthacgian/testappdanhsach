...
// Khôi phục displayHeaders từ sessionStorage nếu có
(function restoreDisplayHeaders() {
  try {
    const saved = sessionStorage.getItem('displayHeaders');
    if (saved) {
      const parsed = JSON.parse(saved);
      if (parsed && parsed.length > 0) {
        displayHeaders = parsed;
        console.log('✅ Đã khôi phục displayHeaders từ sessionStorage:', displayHeaders);
      }
    }
  } catch(e) {
    console.warn('Không thể khôi phục displayHeaders:', e);
  }
})();
...
async function loadAllMembers() {
  if (session.role === "admin") {
    ...
    if (result.success) {
      members = result.data || [];
      // Cập nhật displayHeaders từ server trả về (đã có tên cột mới)
      if (Array.isArray(result.headers) && result.headers.length > 0) {
        ...
        // Lưu vào sessionStorage để dùng sau
        try {
          sessionStorage.setItem('displayHeaders', JSON.stringify(displayHeaders));
        } catch(e) {}
      } else if (members.length > 0) {
        // Nếu server không trả về headers, lấy từ member đầu tiên
...
async function reloadDataFromSheet(evt) {
  ...
  try {
    await loadAllMembers();
    renderTable();
    alert("✅ Dữ liệu đã được tải lại thành công!");
  } catch (err) {
...
async function saveSheetConfig() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Cập nhật displayHeaders với tên cột MỚI từ form
      displayHeaders = headers.filter(k => k !== "__rowIndex" && k !== "MatKhau" && k !== "hasPassword");
      
      // Lưu displayHeaders vào sessionStorage để dùng khi refresh
      try {
        sessionStorage.setItem('displayHeaders', JSON.stringify(displayHeaders));
      } catch(e) {}
      ...
      // Gọi loadAllMembers để lấy dữ liệu mới với headers đã cập nhật
      await loadAllMembers();
      
      renderTable();
      alert("✅ Cập nhật cấu hình sheet thành công! Dữ liệu đã được làm mới.");
      hideLoading();
...
function buildEditForm(record) {
  ...
  // Lấy displayHeaders từ sessionStorage nếu có (ưu tiên)
  ...
  // Sử dụng headers đã có (tên cột mới nhất)
  const liveFields = headers.filter(f => f !== "STT" && f !== "MatKhau" && f !== "__rowIndex" && f !== "hasPassword");
  ...
  fields.forEach(f => {
    ...
    let value = "";
    // Lấy giá trị từ record theo tên cột HIỆN TẠI
...
function openExportModal() {
  ...
  // Lấy displayHeaders từ sessionStorage nếu có
  let headers = displayHeaders;
  try {
    const saved = sessionStorage.getItem('displayHeaders');
    if (saved) {
  ...
  const liveCols = headers.filter(f => f !== "MatKhau" && f !== "__rowIndex" && f !== "hasPassword");
